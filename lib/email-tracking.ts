...
import { getEmailSendById, updateEmailSend, getEmailSends, getEmailSendsByTemplate } from './data-access';
...
export class EmailTrackingService {
  ...
  static async updateDeliveryStatus(sendId: string, userId: string): Promise<TrackingResult | null> {
    try {
      ...
      const emailSend = await getEmailSendById(sendId, userId);
  ...
  static async getDeliveryStats(userId: string, days: number = 30): Promise<{
    ...
    try {
      const emailSends = await getEmailSends(userId);
      ...
      // Calculate totals (using database schema: 'pending' | 'sent' | 'failed')
      ...
      const pending = recentSends.filter(send => 
        send.status === 'pending'
      ).length;
      ...
      return {
        ...
        delivered,
        failed,
        bounced: pending, // Map pending to bounced for backward compatibility
        deliveryRate,
        recentActivity
      };
    } catch (error) {
  ...
  static async getEngagementMetrics(templateId: string, userId: string): Promise<{
    ...
    try {
      const emailSends = await getEmailSendsByTemplate(templateId, userId);
